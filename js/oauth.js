...
function injectKakaoButton(containerId, label) {
    const container = document.getElementById(containerId);
    if (!container) return;
    container.insertAdjacentHTML("beforeend", `
        <div class="oauth-divider"><span>또는</span></div>
        <button type="button" class="btn-kakao" onclick="kakaoLogin()">
            <svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 3C6.48 3 2 6.58 2 11c0 2.79 1.79 5.24 4.5 6.65L5.5 21l3.65-2.42c.93.18 1.89.27 2.85.27 5.52 0 10-3.58 10-8s-4.48-7.85-10-7.85z"/></svg>
            ${label}
        </button>
    `);
}
